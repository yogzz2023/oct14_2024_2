...
def generate_hypotheses(cluster_tracks, cluster_reports):
    # Generate all possible hypotheses for the given cluster
    # Each hypothesis is a list of associations (track_idx, report_idx)
    hypotheses = list(permutations(range(len(cluster_reports)), len(cluster_tracks)))
    return hypotheses

def calculate_hypothesis_probability(hypothesis, cluster_tracks, cluster_reports, kalman_filter):
    # Calculate the probability of a given hypothesis
    cov_inv = np.linalg.inv(kalman_filter.Pp[:3, :3])
    probability = 1.0

    for track_idx, report_idx in enumerate(hypothesis):
        track = cluster_tracks[track_idx]
        report = cluster_reports[report_idx]
        residual = np.array(report) - np.array(track)
        likelihood = np.exp(-0.5 * np.dot(np.dot(residual.T, cov_inv), residual))
        probability *= likelihood

    return probability

def select_best_report_jpda(cluster_tracks, cluster_reports, kalman_filter):
    hypotheses = generate_hypotheses(cluster_tracks, cluster_reports)
    probabilities = []

    for hypothesis in hypotheses:
        prob = calculate_hypothesis_probability(hypothesis, cluster_tracks, cluster_reports, kalman_filter)
    ...
    # Select the best hypothesis based on maximum probability
    best_hypothesis_idx = np.argmax(probabilities)
    best_hypothesis = hypotheses[best_hypothesis_idx]

    # Select the best report for each track based on the best hypothesis
    best_reports = [(track_idx, cluster_reports[report_idx]) for track_idx, report_idx in enumerate(best_hypothesis)]
    return best_reports
...
def perform_jpda(tracks, reports, kalman_filter):
    ...
    for cluster_tracks, cluster_reports in clusters:
        best_reports.extend(select_best_report_jpda(cluster_tracks, cluster_reports, kalman_filter))
    return best_reports
